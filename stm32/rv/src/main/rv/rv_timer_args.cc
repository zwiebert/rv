...
SetArgs::SetArgs(cstr_pair kvs[], int len) {
...
bool SetArgs::to_json(UoutBuilderJson &sj, bool content_only) const {
...
char* SetArgs::toJSON(char *buf, size_t buf_size, bool no_brackets) const {
  const SetArgs &args = *this;
  char *result = buf;
