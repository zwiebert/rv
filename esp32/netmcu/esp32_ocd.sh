...
openocd -f ~/esp/ft2232.cfg -f /usr/local/share/openocd/scripts/board/esp-wroom-32.cfg \
        ...
        -c "program_esp  ota_data_initial.bin $part_ota_init  verify" \
        -c "program_esp  netmcu.bin   $part_ota0 verify reset exit"
}


flash_app() {
openocd -f ~/esp/ft2232.cfg -f /usr/local/share/openocd/scripts/board/esp-wroom-32.cfg \
        -c "program_esp  netmcu.bin  $part_ota0 reset exit"
}
